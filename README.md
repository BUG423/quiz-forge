...
安装或部署后，在“开始练习”卡片中打开设置，点击 **“导入 Excel 文件”**，即可在设备本地形成自己的题库。
...
如果已有多份本地 `.xlsx` 题库，可将它们放入 `data/`（可用子目录分组），然后运行：

```bash
npm run convert:banks
```

脚本为每份源文件在 `data/converted/` 生成一个标准 17 列的独立 `.xlsx` 文件；子目录名会加入题库名称，避免不同专业的同名文件混淆。脚本会用软件实际的导入器重新读取每份转换结果，逐题核对题型、题干、选项、答案、难度、类别、来源、保命标记及案例背景，并在终端报告题数、跳过原因和需要人工复核的源表行号。答案引用了源文件中缺失的选项时，该题会被跳过，避免生成无法正确作答的题目。转换结果只供本地导入使用。
